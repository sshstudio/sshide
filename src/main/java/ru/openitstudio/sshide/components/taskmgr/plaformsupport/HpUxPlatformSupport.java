...
public class HpUxPlatformSupport implements PlatformSupport {
    ...
    public void updateMetrics(SshClient client) throws Exception {
        ...
        exec.setCommand("export PATH=$PATH:/usr/sbin; vmstat 1 2; echo " + DELIMITER + "; swapinfo -ta");
        exec.connect();
...
//    }
    ...
    public void parseVmStatOutput(String text) {
        ...
        if (lastLine != null && colFreeMem != -1 && colActiveMem != -1
            ...
            activeMemory = Long.parseLong(columns[colActiveMem]) * 4096;
            freeMemory = Long.parseLong(columns[colFreeMem]) * 4096;
            ...
            if (totalMemory > 0) {
                this.memoryUsage = ((double) (usedMemory) * 100) / totalMemory;
            }
...
//            }
    ...
    private void parsePstatOutput(String text) {
        ...
        if (matcher.find()) {
            long totalSwap = Long.parseLong(matcher.group(1).trim()) * 1024;
            long usedSwap = Long.parseLong(matcher.group(2).trim()) * 1024;
...
//    }
    ...
    public void updateProcessList(SshClient client) throws Exception {
        String ps = "export UNIX95=1; ps -e -o pid=pid -o pcpu -o vsz -o etime -o ppid -o user -o nice -o args";
        ChannelExec exec = client.getExecChannel();
        ByteArrayOutputStream bout = new ByteArrayOutputStream();
        exec.setOutputStream(bout);
        exec.setCommand(ps);
